...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, context_length, model_dimension, n_heads, dropout = 0.7, device = 'cpu'):
        ...
        # self.q = nn.Linear(self.model_dimension, self.each_head_size , bias=False, device=self.device)
        # self.k = nn.Linear(self.model_dimension, self.each_head_size , bias=False, device=self.device)
        # self.v = nn.Linear(self.model_dimension, self.each_head_size , bias=False, device=self.device)
    ...
    def forward(self,x):
        ...
        # print(f'[INFO-MHSA] {x.shape = } ')
        QKV = self.W_qkv(x)
        # print(f'[INFO-MHSA] {QKV.shape = } ')
        Q = QKV[:,:,:self.model_dimension].view(B, T, self.n_heads, self.each_head_size).transpose(1,2)
        # print(f'[INFO-MHSA] {Q.shape = } ')
        K = QKV[:,:,self.model_dimension:2 * self.model_dimension].view(B, T, self.n_heads, self.each_head_size).transpose(1,2)
        # print(f'[INFO-MHSA] {K.shape = } ')
        V = QKV[:,:,2 * self.model_dimension :].view(B, T, self.n_heads, self.each_head_size).transpose(1,2)
        # print(f'[INFO-MHSA] {V.shape = } ')
        ...
        out = scores @ V
        # print(f'[INFO-MHSA] 1 {out.shape = }')
        out = out.transpose(1,2)
        # print(f'[INFO-MHSA] 2 {out.shape = }')
        out = out.reshape(B, T, C)
        # print(f'[INFO-MHSA] 3 {out.shape = }')
