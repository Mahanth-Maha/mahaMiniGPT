...
class myTokenizer_faster:
    def __init__(self, token_model_file, special_tokens=None, base_tokens = 128):
        self.merges = {}
        self.token_model_file = token_model_file 
        self.base_tokens = base_tokens 
        self.special_tokens = special_tokens if special_tokens else {}
        self.special_tokens_backup = special_tokens if special_tokens else {}
        self.vocab = self._vocab()
        ...
        if vocab_size <= self.base_tokens:
            ...
            try:
                with open(data_file, 'r', encoding='utf-8') as f:
                    ...
                    # Optional: Complete the last word to avoid cutting mid-word
            ...
            for i in tqdm(range(num_merges), desc="BPE Training", unit="merge"):
                while heap:
    ...
    def encode_fast(self, text, progress=False):
        ...
        # Progress bar setup
        if progress:
            # Rough estimate: upper bound on number of merges ~ len(ids)
            pbar = tqdm(total=len(ids), desc="Encoding", unit="merge", dynamic_ncols=True)
            start_time = time.time()

        # Greedy left-to-right merge with a trie-like walk
        i = 0
        out = []
        merges_applied = 0
        while i < len(ids):
            ...
            # Try to match longest merge sequence
    ...
    def _vocab(self, vocab_size=None):
        ...
        if vocab_size is not None and vocab_size <= self.base_tokens:
            vocab = {idx: bytes([idx]) for idx in range(vocab_size)}
            return vocab
        else:
            vocab = {idx: bytes([idx]) for idx in range(self.base_tokens)}
    ...
    def encode_production(self, text, progress=False):
        ids = list(text.encode("utf-8"))
        if len(ids) <= 1:
            return ids
        ...
        if progress:
            estimated_iterations = min(len(self.merges), initial_length // 4)
            pbar = tqdm(total=estimated_iterations, desc="Encoding ", unit="merges", miniters=1, dynamic_ncols=True)
        
        # STEP 2: Main BPE loop - identical to tiktoken/sentencepiece
        while len(ids) >= 2:
            # Find all consecutive pairs and their merge priorities
            pairs = {}
            for i in range(len(ids) - 1):
                pair = (ids[i], ids[i + 1])
                if pair in self.merges:
                    # Store the LOWEST priority value (highest priority merge)
                    if pair not in pairs or self.merges[pair] < pairs[pair]:
                        pairs[pair] = self.merges[pair]
            
            # No more merges possible
            if not pairs:
                break
            
            # CRITICAL: Find the pair with LOWEST merge index (applied first during training)
            best_pair = min(pairs.items(), key=lambda item: item[1])[0]
            
            # Apply the merge in a single pass
            new_ids = []
            i = 0
            while i < len(ids):
                # Check if current position forms the best pair
                if (i < len(ids) - 1 and 
                    ids[i] == best_pair[0] and 
                    ids[i + 1] == best_pair[1]):
                    # Merge the pair
            ...
            if progress:
                pbar.update(1)
                # Minimal postfix updates for speed
                if pbar.n % 50 == 0:
                    compression = len(ids) / initial_length
                    pbar.set_postfix(tokens=len(ids), ratio=f"{compression:.3f}x")
        
        if progress:
            pbar.close()
            final_compression = len(ids) / initial_length
            print(f"✅ Encoded: {initial_length:,} → {len(ids):,} tokens "
                f"({final_compression:.3f}x compression)")
        
        return ids

    def encode_ultra_optimized(self, text, progress=False):
        """Ultra-optimized version with all production tricks"""
        from tqdm import tqdm
        ...
        initial_length = len(ids)
        merge_dict = self.merges  # Pre-compute for O(1) lookups
        
        if progress:
            pbar = tqdm(desc="⚡ Ultra-Optimized", unit="merges", 
                    miniters=10, mininterval=0.5)
        
        iteration = 0
        
        while len(ids) >= 2:
            # OPTIMIZATION: Single pass to find best merge
            ...
            # Apply merge with minimal memory allocation
            merge_token = merge_dict[best_pair]
            new_ids = []
            i = 0
        ...
        if progress:
            pbar.n = iteration
            pbar.close()
            print(f"⚡ Ultra: {initial_length:,} → {len(ids):,} tokens ({iteration} merges)")
