...
from .TBlock import TBlockRPreLNwDropout
from .NgramModel import NGramLanguageModel


class SRBlockPreLNwDropout(nn.Module):
    def __init__(self, vocab_size, block_size, Nx = 2 , ffn_hid_dim = 258, n_heads = 8, n_embeddings =128, dropout = 0.7,device = 'cpu', nonlinearity ='gelu', n_gram = 2):
        super(SRBlockPreLNwDropout, self).__init__()
        self.Nx = Nx
        self.blocks = nn.ModuleList(
            [
                TBlockRPreLNwDropout(
...
class NGramSRBlocksPreLNwDP(NGramLanguageModel):

    def __init__(self, vocab_size, block_size, Nx = 2 , ffn_hid_dim = 258, n_heads = 8, n_embeddings =128, dropout = 0.7, device = 'cpu', nonlinearity ='gelu', n_gram = 2):
        super(NGramSRBlocksPreLNwDP, self).__init__( vocab_size, block_size ,n_embeddings = n_embeddings, device = device, n_gram = n_gram)
        ...
        self.stacked_blocks = SRBlockPreLNwDropout(
                    vocab_size=vocab_size,
                    block_size=block_size,
                    Nx=Nx,
                    ffn_hid_dim=ffn_hid_dim,
