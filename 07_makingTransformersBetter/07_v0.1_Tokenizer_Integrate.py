...
def main():
    ...
    if os.path.exists(args.encoding_file):
        ...
        encoded_data = torch.tensor(
            tokenizer.encode_production(data[:int(len(data)*0.1)], progress=True),
            dtype=torch.long, 
            device=DEVICE
        )
    ...
    logger_transformer.info(f"Dataset have {len(encoded_data)} ({convert_to_hr(len(encoded_data))}) tokens out of which train tokens are {len(encoded_data)*0.8} \t({convert_to_hr(len(encoded_data)*0.8)})") 
    train_time = None
    if args.train:
