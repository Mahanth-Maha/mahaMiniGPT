...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, context_length, model_dimension, n_heads, n_kv_heads=None, dropout=0.7, device='cpu'):
        super().__init__()
        # print(f'[INFO-MHSA] {self.variable_name.shape = } \t)')
        ...
        self.W_q = nn.Linear(model_dimension, n_heads * self.each_head_size, bias=False, device=device)
        self.W_k = nn.Linear(model_dimension, self.n_kv_heads * self.each_head_size, bias=False, device=device)
        self.W_v = nn.Linear(model_dimension, self.n_kv_heads * self.each_head_size, bias=False, device=device)
    ...
    def forward(self,x):
        ...
        Q = self.W_q(x).view(B, T, self.n_heads, self.each_head_size).transpose(1, 2)
        K = self.W_k(x).view(B, T, self.n_kv_heads, self.each_head_size).transpose(1, 2)
        V = self.W_v(x).view(B, T, self.n_kv_heads, self.each_head_size).transpose(1, 2)
...
class FeedForwardNet(nn.Module):
    def __init__(self, model_dimension, ffn_hid_dim = None, non_linearity = 'gelu', dropout = 0.7, device = 'cpu'):
        super().__init__()
        # print(f'[INFO-FFN] {self.variable_name.shape = }')
...
class DecoderBlock(nn.Module):
    def __init__(self, context_length, model_dimension, n_heads, ffn_hid_dim = None, n_kv_heads = None,  non_linearity ='gelu', dropout = 0.7, device = 'cpu'):
        super().__init__()
        # print(f'[INFO-DECODER] {self.variable_name.shape = }')
        ...
        self.attention = MultiHeadSelfAttention(self.context_length, self.model_dimension, self.n_heads, self.n_kv_heads, self.dropout, self.device)
...
class DecoderOnlyTransformer(nn.Module):
    ...
    def __init__(self, vocab_size, context_length, model_dimension, n_heads, Nx, ffn_hid_dim = None, n_kv_heads= None, non_linearity ='gelu', dropout = 0.7, device = 'cpu'):
        super().__init__()
        # print(f'[INFO-Transformer] {self.variable_name.shape = }')
        ...
        print(f'[DEBUG] Before init | tok embedding: mean={torch.mean(self.token_embeddings.weight):.4f} std={torch.std(self.token_embeddings.weight):.4f}')
        self.apply(self._init_weights)
        print(f'[DEBUG] After init  | tok embedding: mean={torch.mean(self.token_embeddings.weight):.4f} std={torch.std(self.token_embeddings.weight):.4f}')
...
def get_args():
    ...
    parser.add_argument("--n-embeddings", type=int, default=128, help="model dimension")
    parser.add_argument("--n-kv-heads", type=int, default=4, help="n KV heads : in Grouped Query attn")
...
def main():
    ...
    if args.check:
        ...
        options = dict(
            ...
            model_dimension =args.n_embeddings,
    ...
    if args.train:
        ...
        for iter in pbar:
            ...
            pbar.set_postfix({
                ...
                'val_ppl': f'{val_perplexity if val_perplexity < 1e8 else float("inf") :.2f}',
