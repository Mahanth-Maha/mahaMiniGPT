...
class DecoderOnlyTransformer(nn.Module):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            torch.nn.init.normal_(module.weight, mean=0.0, std=std_val * ((2 * self.Nx) ** -0.5))
            if module.bias is not None:
                torch.nn.init.zeros_(module.bias)
        elif isinstance(module, nn.Embedding):
            torch.nn.init.normal_(module.weight, mean=0.0, std=std_val)
    ...
    def forward(self, x):
        ...
        for i, block in enumerate(self.blocks):
            if i >= len(self.blocks) // 2:
                embeds = checkpoint(block, embeds)
            else:
                embeds = block(embeds)
...
def get_args():
    ...
    parser.add_argument("--encoding-file", default='/mnt/volume5tb/data/english/WanJuan_cc/OpenDataLab___WanJuanCC/raw/shards/part-659f9e2120e3-000622/shard_00000.pt', help="Encoded data file")
...
def main():
    ...
    EVAL_ITERS = min(MAX_ITERS // 1000, 2500)
    if EVAL_ITERS == 0:
        ...
        for iter in pbar:
            ...
            if iter % EVAL_ITERS == 0 or iter == 0:
                ...
                with torch.no_grad():
                    ...
                    loss = F.cross_entropy(val_logits.view(-1, VOCAB_SIZE), val_y.view(-1))
            ...
            pbar.set_postfix({
                'loss': f'{loss.item():.4f}' if loss.item() > 1.0 else f'{loss.item():.8f}' ,
            ...
            if loss.item() < 1e-4:
                logger_transformer.info(f'🤷 Training Stopped: Loss ({loss.item()}) is too low >>> MAY BE MODEL COLLAPSED <<<')
                break
    ...
    if args.generate:
        ...
        '\nGenerated: ' + tokenizer.decode(
            model.generate(torch.zeros((1, 1), dtype=torch.long, device=DEVICE),
                           max_pred_tokens=MAX_CONTEXT_LEN,
                           temp=1.0,
                           top_k=50,
                           top_p=0.9,
                           )[0].tolist()) + '\n'
