...
optimizations TODO:
    ...
    2. Positional Embeddings (Effect: Quality++ -> embedding highers ctx)
    ...
    5. Better Normalization (Effect: Quality++ )
        - Sinusoidal like the Transformer paper
        - RoPE
...
class SingleHeadSelfAttention(nn.Module):
    def __init__(self, n_embeddings , head_size, context_length, vocab_size, dropout = 0.7, device = 'cpu'):
        ...
        self.device = device
        self.attn_norm = 1/(self.head_size ** 0.5)
        
        self.q = nn.Linear(self.n_embeddings, self.head_size , bias=False).to(self.device)
        self.k = nn.Linear(self.n_embeddings, self.head_size , bias=False).to(self.device)
        self.v = nn.Linear(self.n_embeddings, self.head_size , bias=False).to(self.device)
...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, vocab_size, context_length, n_embeddings, n_heads, dropout = 0.7, device = 'cpu'):
        ...
        self.each_head_size = self.n_embeddings //n_heads, 
        ...
        self.proj_attn = nn.Linear(self.n_embeddings,self.n_embeddings)
...
class FeedForwardNet(nn.Module):
    def __init__(self, n_embeddings, ffn_hid_dim = None, non_linearity = 'gelu', dropout = 0.7, device = 'cpu'):
        ...
        self.proj_ffn = nn.Linear(self.ffn_hid_dim, self.n_embeddings).to(self.device)
        self.dropout_layer = nn.Dropout(self.dropout)
...
class DecoderBlock(nn.Module):
    def __init__(self, vocab_size, context_length, n_embeddings, n_heads, ffn_hid_dim = None,  non_linearity ='gelu', dropout = 0.7, device = 'cpu'):
        ...
        self.attention = MultiHeadSelfAttention(self.vocab_size, self.context_length, self.n_embeddings, self.n_heads, self.dropout, self.device)
...
class DecoderOnlyTransformer(nn.Module):
    ...
    def __init__(self, vocab_size, context_length, n_embeddings, n_heads, Nx, ffn_hid_dim = None, non_linearity ='gelu', dropout = 0.7, device = 'cpu'):
        ...
        self.token_embeddings = nn.Embedding(self.vocab_size, self.n_embeddings)
        self.position_embeddings = nn.Embedding(self.context_length, self.n_embeddings)
        ...
        self.lm_head = nn.Linear(self.n_embeddings, self.vocab_size)

    def forward(self, x, y=None):
        xB , xT = x.shape
        xtokens = self.token_embeddings(x)
        xpos = self.position_embeddings(torch.arange(xT).to(self.device))
        embeds = xtokens + xpos
