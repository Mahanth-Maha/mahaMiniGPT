...
class DecoderOnlyTransformer(nn.Module):
    ...
    def forward(self, x, y=None, verbose = False):
        ...
        if verbose:
            print(f'[DEBUG][dim] {logits.shape = }')
...
if __name__ == '__main__':
    ...
    print(f'[INFO][dim] {random_X.shape = } {random_y.shape = }')
    
    # logit = gpt(random_X, verbose=True)
    
    # print(f'[INFO][dim] {logit.shape = }')
    
    logit, loss = gpt(random_X, random_y, verbose=True)
    
    print(f'[INFO][dim] {logit.shape = } {loss.data = }')