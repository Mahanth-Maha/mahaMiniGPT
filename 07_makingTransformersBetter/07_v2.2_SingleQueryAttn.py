...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, context_length, model_dimension, n_heads, dropout = 0.7, device = 'cpu'):
        ...
        self.W_qkv = nn.Linear(self.model_dimension, 3 * self.each_head_size , bias=False, device=self.device)
    ...
    def forward(self,x):
        ...
        Q = QKV[:,:,:self.each_head_size].unsqueeze(-1).expand(-1,-1,-1,self.n_heads).transpose(-2,-1).transpose(1,2)
        K = QKV[:,:,self.each_head_size:2 * self.each_head_size].unsqueeze(-1).expand(-1,-1,-1,self.n_heads).transpose(-2,-1).transpose(1,2)
        V = QKV[:,:,2 * self.each_head_size :].unsqueeze(-1).expand(-1,-1,-1,self.n_heads).transpose(-2,-1).transpose(1,2)
