...
from .TBlock import TBlockResidualsProj
from .NgramModel import NGramLanguageModel


class StackedResidualBlocksProj(nn.Module):
    def __init__(self, vocab_size, block_size, Nx = 2 , ffn_hid_dim = 258, n_heads = 8, n_embeddings =128, device = 'cpu', nonlinearity ='gelu', n_gram = 2):
        super(StackedResidualBlocksProj, self).__init__()
        self.Nx = Nx
...
class NGramSRBlocksProj(NGramLanguageModel):

    def __init__(self, vocab_size, block_size, Nx = 2 , ffn_hid_dim = 258, n_heads = 8, n_embeddings =128, device = 'cpu', nonlinearity ='gelu', n_gram = 2):
        super(NGramSRBlocksProj, self).__init__( vocab_size, block_size ,n_embeddings = n_embeddings, device = device, n_gram = n_gram)
        ...
        self.stacked_blocks = StackedResidualBlocksProj(
                    vocab_size=vocab_size,
                    block_size=block_size,
                    Nx=Nx,
                    ffn_hid_dim=ffn_hid_dim,
